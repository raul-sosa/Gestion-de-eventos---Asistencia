...
function StudentDashboard() {
  ...
  const loadMyRegistrations = async () => {
    ...
    try {
      ...
      const preRegRes = await axios.get(`/pre-registros/student/${userId}`);
  ...
  const openPreRegisterModal = (event) => {
    if (!user) {
  ...
  const handlePreRegisterConfirm = async (matricula) => {
    if (!user || !selectedEvent) return;

    try {
      await axios.post("/pre-registros", {
        id_evento: selectedEvent.id,
        id_estudiante: user.user_id || user.id || user.username,
        matricula: matricula,
      });

      alert(
        `✓ Pre-registro exitoso!\n\nEvento: ${selectedEvent.nombre}\nMatrícula: ${matricula}\n\nPor favor espera la confirmación del encargado.`
      );
      ...
      throw new Error(
        error.response?.data?.detail || "Error al pre-registrarse"
      );
    }
  };
  ...
  return (
    <div className="student-dashboard">
            ...
            {events.map((event) => {
              ...
              return (
                <div key={event.id} className="event-card-student">
                  ...
                  <button
                    onClick={() => openPreRegisterModal(event)}
      ...
      <PreRegisterModal
        ...
        onConfirm={handlePreRegisterConfirm}
